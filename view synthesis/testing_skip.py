...
def main(cfg: dataset['nodename']):
    ...
    if cfg.settings.wandb==True:
        ...
        artifact = run.use_artifact(f'ldenridder/Autoencoder (1.1.2)/SWINKITTI_lr-{cfgdir.hyperparams.base_lr}_bs-{cfgdir.hyperparams.batch_size}_epoch-test:v8', type='model')
        artifact.download()
    ...
    model = model_loader_skip(cfg, cfgdir, device, skip)
    checkpoint = torch.load(f'artifacts/SWINKITTI_lr-{cfgdir.hyperparams.base_lr}_bs-{cfgdir.hyperparams.batch_size}_epoch-test:v8/tmp_model.pth')
