...
def main(cfg: dataset['nodename']):
    ...
    if cfg.settings.wandb==True:
        ...
        artifact = run.use_artifact(f'ldenridder/Autoencoder (1.1.2)/SWINKITTI_lr-{cfgdir.hyperparams.base_lr}_bs-{cfgdir.hyperparams.batch_size}_epoch-test:v11', type='model')
        artifact.download()
    ...
    # model_1 = model_loader_test(cfg, cfgdir, device,5)
    # model_2 = model_loader_test(cfg, cfgdir, device,6)
    # model_3 = model_loader_test(cfg, cfgdir, device,7)
    # model_all = model_loader_test(cfg, cfgdir, device,8)
    checkpoint = torch.load(f'artifacts/SWINKITTI_lr-{cfgdir.hyperparams.base_lr}_bs-{cfgdir.hyperparams.batch_size}_epoch-test:v11/tmp_model.pth')
    ...
    with torch.no_grad():
        ...
        for i, (left_images, right_images) in enumerate(test_loader):
            ...
            if i == 0:
                rendering_disparities(left_images, right_images, left_images_prediction, right_images_prediction, cfgdir)
